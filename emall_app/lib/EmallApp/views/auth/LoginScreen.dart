...
import 'package:emall_app/EmallApp/utils/Validator.dart';
import 'package:emall_app/EmallApp/views/AppScreen.dart';
import 'package:emall_app/EmallApp/views/auth/ForgotPasswordScreen.dart';
import 'package:emall_app/EmallApp/views/auth/RegisterScreen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  _checkUserLoginOrNot() async {
    if (await AuthController.isLoginUser()) {
      // check if result not nullmaybe
      /// Push the given route onto the navigator that most tightly encloses
      /// the given context, and then remove all the previous routes until
  ...
  _handleLogin() async {
    ...
    if (email.isEmpty) {
      ...
      if (response.success) {
        ...
        /// checkRedirectNavigation(): return switch to check evry case
        /// of code status ane return the equivalent route
                    ...
                    child: GestureDetector(
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            /// base in _handleFB methode to make an http post
                            /// with valide email and from the backend (server)
