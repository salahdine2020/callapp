...
import 'package:admin_super/models/Account.dart';
import 'package:admin_super/models/Manager.dart';
import 'package:admin_super/models/MyResponse.dart';
import 'package:admin_super/services/PushNotificationsManager.dart';
import 'package:admin_super/utils/InternetUtils.dart';
import 'package:admin_super/utils/SizeConfig.dart';
...
class AuthController {
  ...
  static Future<MyResponse> loginUser(String email, String password) async {
    ...
    //URL
    String loginUrl =
        ApiUtil.MAIN_API_URL + ApiUtil.AUTH_LOGIN; //  AUTH_LOGIN = "login";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Map<String, dynamic> manager = data['manager'];
        String token = data['token'];

        await saveUser(manager);
  ...
  //------------------------ Save user in cache -----------------------------------------//
  static saveUser(Map<String, dynamic> manager) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    await sharedPreferences.setString('name', manager['name']);
    await sharedPreferences.setString('email', manager['email']);
    await sharedPreferences.setString('avatar_url', manager['avatar_url']);
  }

  static saveUserFromManager(Manager manager) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    await sharedPreferences.setString('name', manager.name);
    await sharedPreferences.setString('email', manager.email);
    await sharedPreferences.setString('avatar_url', manager.avatarUrl);
  }
