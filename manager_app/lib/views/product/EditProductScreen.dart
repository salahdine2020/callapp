...
import 'package:manager_app/views/product/EditProductImageScreen.dart';
...
class _EditProductScreenState extends State<EditProductScreen> {
  ...
  _fetchProduct() async {
    ...
    MyResponse<Product> myResponse = await ProductController.getProduct(widget.id);
    if (myResponse.success) {
  ...
  _createProduct() async {
    ...
    /// fill list of items by json object
    String items = "[";
              ...
              onPressed: () {
                setState(() {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (BuildContext context) => EditProductImageScreen(
                        id: widget.id,
                      ),
                    ),
                  );
...
class _ProductItemWidgetState extends State<ProductItemWidget> {
  ...
  void initState() {
    ...
    tePrice = TextEditingController(
        text: widget.isFilled ? productItem.price.toString() : "");
    teRevenue = TextEditingController(
        text: widget.isFilled ? productItem.revenue.toString() : "");
    teQuantity = TextEditingController(
        text: widget.isFilled ? productItem.quantity.toString() : "");
    ...
    if (widget.isFilled) {
      for (ProductItemFeature productItemFeature
          in productItem.productItemFeatures) {
...
class _ProductItemFeatureWidgetState extends State<ProductItemFeatureWidget> {
  ...
  void initState() {
    ...
    teFeatureValue = new TextEditingController(
        text: widget.isFilled ? productItemFeature.value : "");
    if (!widget.isFilled)
