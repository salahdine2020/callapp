...
class ProductItem {
 ...
 bool isValid(){
    ...
    for(ProductItemFeature productItemFeature in productItemFeatures){
      if(!productItemFeature.isValid())
        return false;
    }
    return price!=null && revenue!=null && quantity!=null && productItemFeatures.length!=0;
  }

  String toJSON(){
    String data = "{\"product_item_features\":[";
    for(int i=0;i<productItemFeatures.length;i++){
      data+=productItemFeatures[i].toJSON();
      if(i+1!=productItemFeatures.length)
        data+=",";
    }
    data+="],\"price\":\""+price.toString()+"\",\"quantity\":\""+quantity.toString()+"\",\"id\":\""+id.toString()+"\",\"revenue\":\""+revenue.toString()+"\"}";
    return data;
  }
}
